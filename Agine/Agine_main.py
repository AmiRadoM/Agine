...
class GameObject():
    ...
    def __setattr__(self, key, value):
        ...
        try:
            updateFunctions.add(callable(getattr(value,"update")))
        except:
            pass
...
def clamp(value ,maxValue):
    return max(0, min(maxValue, value-1))
...
def renderer():
    ...
    for cam in objects:
        if(hasattr(cam, "Camera")):
            for object in objects + debugObjects:
                if object.isVisible:
                    #Outline
            ...
            for object in objects + debugObjects:
                if object.isVisible:
                    ...
                    # 2D !!!
                    if type(object) != Circle and type(object) != Polygon and type(object) != Line:
                        newScale = (object.Transform.scale * gameDisplay.scale / 2) / cam.Camera.scale
                        newPos = cam.Camera.TranslateWorldVector2D(Vector2D(
                            object.Transform.position.x - object.Transform.scale.x / 2 - cam.Transform.position.x,
                            object.Transform.position.y + object.Transform.scale.y / 2 - cam.Transform.position.y))

                    if (hasattr(object, "Line")):
                        startVector = cam.Camera.TranslateWorldVector2D(object.Line.startPoint)
                        endVector = cam.Camera.TranslateWorldVector2D(object.Line.endPoint)
                        pygame.draw.line(gameDisplay.display, object.Line.color, [startVector.x, startVector.y],
                                         [endVector.x, endVector.y], object.Line.width)

                    if (hasattr(object, "Square")):
                        pygame.draw.rect(gameDisplay.display, object.Square.color,
                                         (newPos.x, newPos.y, newScale.x, newScale.y),
                                         object.Square.width)

                    if hasattr(object, "Sprite"):
                        if (object.Sprite.imagePath != None):
                            img = pygame.image.load(object.Sprite.imagePath)
                            ...
                            colorImage.fill(object.Sprite.color)
                    ...
                    if hasattr(object, "Polygon"):
                        allX = [point.x for point in object.Polygon.points]
                        allY = [point.y for point in object.Polygon.points]
                        allX = [x * object.Transform.scale.x for x in allX]
                        allY = [y * object.Transform.scale.y for y in allY]
                        avarageX = sum(allX) / len(allX)
                        avarageY = sum(allY) / len(allY)

                        deltaX = (object.x - avarageX)
                        deltaY = (object.y - avarageY)

                        translatedPoints = [Vector2D(
                            (allX[i] + deltaX) + gameDisplay.display.get_width() / 2 - cam.Transform.position.x, -(
                                    (allY[
                                         i] + deltaY) - gameDisplay.display.get_height() / 2 - cam.Transform.position.y))
                                            for i in
                                            range(len(allX))]

                        pygame.draw.polygon(gameDisplay.display, object.Polygon.color, translatedPoints,
                                            object.Polygon.width)

                    if hasattr(object, "Circle"):
                        cPos = cam.Camera.TranslateWorldVector2D(object.Transform.position)
                        pygame.draw.circle(gameDisplay.display, object.Circle.color, (cPos.x, cPos.y),
                                           object.Circle.radius, object.Circle.width)
                    ...
                    if (hasattr(object, "Cube") or hasattr(object, "Mesh")):
                        ...
                        # Scaling Matrix
                        matrixScale = Matrix4x4.MakeScale(object.Transform.scale)

                        # Translation Matrix and World Matrix
                        matrixTranslation = Matrix4x4.MakeTranslation(object.Transform.position.x,
                                                                      object.Transform.position.y,
                                                                      object.Transform.position.z)
                        matrixWorld = matrixRotationZ * matrixRotationX * matrixRotationY * matrixScale
                        ...
                        for tri in object.Mesh.triangles if hasattr(object, "Mesh") else object.Cube.triangles:
                            ...
                            cameraRay = triTransformed.points[0] - Vector3D(cam.Transform.position.x,
                            ...
                            if (normal.DotProduct(cameraRay) < 0):
                                ...
                                max(min(255 * dotProduct, 255), 0), max(min(255 * dotProduct, 255), 0),
                                max(min(255 * dotProduct, 255), 0))
                                ...
                                clipped = [None, None]
                                clipped[0], clipped[1] = triViewed.ClipAgainstPlane(Vector3D(0, 0, 0.1),
                                                                                    Vector3D(0, 0, 1))
                                for clippedTri in clipped:
                                    if (clippedTri != None):
                                        ...
                                        triProjected.textureCords[0].x = triProjected.textureCords[0].x / \
                                                                         triProjected.points[0].w
                                        triProjected.textureCords[1].x = triProjected.textureCords[1].x / \
                                                                         triProjected.points[1].w
                                        triProjected.textureCords[2].x = triProjected.textureCords[2].x / \
                                                                         triProjected.points[2].w

                                        triProjected.textureCords[0].y = triProjected.textureCords[0].y / \
                                                                         triProjected.points[0].w
                                        triProjected.textureCords[1].y = triProjected.textureCords[1].y / \
                                                                         triProjected.points[1].w
                                        triProjected.textureCords[2].y = triProjected.textureCords[2].y / \
                                                                         triProjected.points[2].w
                        ...
                        #Split Triangles to Smaller Triangles
                        for triToRender in trianglesToRender:
                            ...
                            for p in range(4):
                                while (numOfTris > 0):
                                    ...
                                    trisToAdd = [None, None]
                                    if (p == 0):
                                        trisToAdd[0], trisToAdd[1] = test.ClipAgainstPlane(Vector3D(0, 0, 0),
                                                                                           Vector3D(0, 1, 0))
                                    elif (p == 1):
                                        trisToAdd[0], trisToAdd[1] = test.ClipAgainstPlane(
                                            Vector3D(0, gameDisplay.scale.y - 1, 0), Vector3D(0, -1, 0))
                                    elif (p == 2):
                                        trisToAdd[0], trisToAdd[1] = test.ClipAgainstPlane(Vector3D(0, 0, 0),
                                                                                           Vector3D(1, 0, 0))
                                    elif (p == 3):
                                        trisToAdd[0], trisToAdd[1] = test.ClipAgainstPlane(
                                            Vector3D(gameDisplay.scale.x - 1, 0, 0), Vector3D(-1, 0, 0))
                            ...
                            for newTri in listTriangles:
                                ...
                                img = object.Cube.texture if (hasattr(object, "Cube")) else object.Mesh.texture

                                x1 = int(newTri.points[0].x)
                                y1 = int(newTri.points[0].y)
                                u1 = newTri.textureCords[0].x
                                v1 = newTri.textureCords[0].y
                                w1 = newTri.textureCords[0].w

                                x2 = int(newTri.points[1].x)
                                y2 = int(newTri.points[1].y)
                                u2 = newTri.textureCords[1].x
                                v2 = newTri.textureCords[1].y
                                w2 = newTri.textureCords[1].w

                                x3 = int(newTri.points[2].x)
                                y3 = int(newTri.points[2].y)
                                u3 = newTri.textureCords[2].x
                                v3 = newTri.textureCords[2].y
                                w3 = newTri.textureCords[2].w
                                ...
                                if (dya != 0):
                                    ...
                                    for i in range(y1, y2 + 1):
                                        ...
                                        for j in range(firstX, lastX + 1):
                                            ...
                                            color = img.get_at((int((1 - texU / texW) * (img.get_size()[0] - 0.00001)),
                                                                int(texV / texW * (img.get_size()[1] - 0.00001))))
                                            clampedI = clamp(i, gameDisplay.scale.y - 1)
                                            clampedJ = clamp(j, gameDisplay.scale.x - 1)
                                            if (texW > depthBuffer[clampedI * gameDisplay.scale.x + clampedJ]):
                                                # gameDisplay.display.set_at((j, i), (color[0] * (tri.color[0] / 255) , color[1] * (tri.color[1] / 255), color[2] * (tri.color[2] / 255 )))
                                                pixelArray[clampedJ, clampedI] = (
                                                color[0] * (newTri.color[0] / 255), color[1] * (newTri.color[1] / 255),
                                                color[2] * (newTri.color[2] / 255))
                                                # newPixels[50][50] = (color[0] * (tri.color[0] / 255), color[1] * (tri.color[1] / 255), color[2] * (tri.color[2] / 255))
                                                depthBuffer[clampedI * gameDisplay.scale.x + clampedJ] = texW
                                            t += tStep
                                ...
                                # pygame.draw.polygon(gameDisplay.display, (0, 0, 0), (
                                # (newTri.points[0].x, newTri.points[0].y), (newTri.points[1].x, newTri.points[1].y),
                                # (newTri.points[2].x, newTri.points[2].y)), 1)
                    ...
                    if (hasattr(object, "Text")):
                        ...
                        newPosUI = Vector2D(object.Transform.position.x * (
                                    gameDisplay.scale.x / 2 / cam.Camera.scale) - text.get_rect().width / 2 + gameDisplay.scale.x / 2,
                                            object.Transform.position.y * (
                                                        gameDisplay.scale.y / 2 / cam.Camera.scale) + text.get_rect().height / 2 - gameDisplay.scale.y / 2)
    ...
    pixelArr = pygame.PixelArray(gameDisplay.display)
    for i in range(gameDisplay.scale.y):
        for j in range(gameDisplay.scale.x):
            if (newPixels[j][i] != None):
                print((j,i))
                pixelArr[j,i] = newPixels[j][i]
    pixelArr.close();
    debugObjects.clear()
...
def Main():
    ...
    while not crashed.get("crashed"):
        ...
        newPixels = [[None] * gameDisplay.scale.y] * gameDisplay.scale.x
...
objects = []
debugObjects = []
depthBuffer = [0] * (gameDisplay.scale.x * gameDisplay.scale.y)
newPixels = [[None] * gameDisplay.scale.y] * gameDisplay.scale.x
